...
, withDNSCrypt ? false
, withDNSTAP ? false
, withEvent ? (!stdenv.hostPlatform.isMinGW)
, withTFO ? false
, withRedis ? false
...
, withDisableFtlo ? stdenv.hostPlatform.isStatic || stdenv.hostPlatform.isMinGW
, libnghttp2
...
stdenv.mkDerivation (finalAttrs: {
  ...
  nativeBuildInputs = [ pkg-config ]
    ++ lib.optionals (!stdenv.hostPlatform.isMinGW) [ makeWrapper ]
  ...
  configureFlags = [
    ...
    "--enable-pie"
    "--enable-relro-now"
  ] ++ lib.optionals withEvent [
    "--with-libevent=${libevent.dev}"
  ] ++ lib.optionals withDisableFtlo [
    "--disable-flto"
